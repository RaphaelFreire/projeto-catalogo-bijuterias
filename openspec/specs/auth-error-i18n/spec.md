...
O processo de implementação SHALL identificar todos os códigos de erro possíveis retornados por `POST /auth/register` no campo `errors[]` da `ApiErrorResponse`, lendo `apps/backend/src/modules/auth/user/auth.integration.http` e `apps/backend/src/shared/errors/api-exception.filter.ts`. A lista identificada SHALL ser registrada na evidência da task correspondente.
