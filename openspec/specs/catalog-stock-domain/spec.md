...
## Purpose
TBD - created by change-006-catalogo-categoria-estoque-imagem. Extended by change-009-pedido-desconto-estoque (decremento atômico condicional). Update Purpose after archive.
...
### Requirement: Contrato do repositório de `stock`, incluindo decremento atômico condicional

O agregado `stock` SHALL expor uma interface de repositório no módulo `catalog` cobrindo as operações necessárias para o CRUD completo: persistir (criar/atualizar), excluir por id, buscar por id e listar paginado, além de `decrementIfAvailable(productId: string, quantity: number): Promise<boolean>`. Esta interface NÃO PODE ser alterada por implementações técnicas. `decrementIfAvailable` SHALL ser implementada de forma atômica (uma única operação condicional no nível do banco, não uma leitura seguida de escrita em passos separados) — retorna `true` quando havia estoque suficiente e o decremento foi aplicado, `false` caso contrário (nada é alterado).

#### Scenario: Contrato cobre operações do CRUD e o decremento condicional

- **WHEN** o contrato é inspecionado
- **THEN** ele expõe operações para persistir, excluir por id, buscar por id, listar paginado e `decrementIfAvailable`
