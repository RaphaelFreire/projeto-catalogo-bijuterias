...
## Purpose
TBD - created by archiving change change-005-cadastro-produto. Extended by change-006-catalogo-categoria-estoque-imagem (categoria e galeria de imagens) e change-007-vitrine-publica-whatsapp (quantidade em estoque no formulário, upload de imagem em modo criação, flags de marketing renomeadas). Update Purpose after archive.
...
### Requirement: Formulário compartilhado em seções, incluindo categoria, quantidade em estoque e imagens
...
- **"Preço e status"**: `preço`, `quantidade em estoque` (número inteiro, não-negativo), `status` como `select` com as opções `active`, `inactive`, `draft`.
- **"Marketing"** (renomeada de "Disponibilidade"): checkboxes `bestSeller` ("Mais vendido"), `dailyDeal` ("Oferta do dia"), `lastUnits` ("Últimas unidades") — selos de marketing exibidos como badge no card da vitrine pública (ver `storefront-frontend`), sem relação com disponibilidade real de estoque.
- **"Imagens"**: galeria de imagens (upload múltiplo, miniaturas, remoção individual) — habilitada em **criação e edição**.
...
#### Scenario: Campo de quantidade em estoque

- **WHEN** o formulário de produto é renderizado (criação ou edição)
- **THEN** o campo "Quantidade em estoque" é exibido na seção "Preço e status", com valor inicial `0` em modo criação e o valor atual do estoque em modo edição
- **AND** ao submeter, o valor é enviado como `quantity` no corpo da requisição a `/products`

#### Scenario: Checkboxes de marketing independentes

- **WHEN** o usuário marca/desmarca `bestSeller`, `dailyDeal` ou `lastUnits`
...
#### Scenario: Seção de imagens disponível também em modo criação

- **WHEN** o formulário é renderizado em modo criação
- **THEN** a seção "Imagens" é exibida, permitindo selecionar arquivos antes de o produto existir
- **AND** os arquivos selecionados ficam em estado local (com preview) até a submissão do formulário

- **WHEN** o formulário é renderizado em modo edição
- **THEN** a seção "Imagens" é exibida com a galeria atual do produto, e upload/remoção acontecem imediatamente (sem esperar a submissão do formulário)

#### Scenario: Upload de imagem em modo edição

- **WHEN** o usuário seleciona um arquivo na seção "Imagens" em modo edição
- **THEN** o frontend chama `POST /products/:id/images` imediatamente
- **AND** ao receber sucesso, a nova miniatura aparece na galeria

#### Scenario: Upload de imagem em modo criação

- **WHEN** o usuário seleciona arquivos na seção "Imagens" em modo criação e depois submete o formulário
- **THEN** o frontend primeiro chama `POST /products` (que retorna `{ id }`) e, em seguida, chama `POST /products/:id/images` para cada arquivo selecionado, usando o `id` retornado
- **AND** falha ao enviar uma imagem específica não impede a criação do produto (já concluída) — o frontend exibe um erro pontual para o arquivo que falhou

#### Scenario: Remoção de imagem

- **WHEN** o usuário remove uma imagem da galeria em modo edição
...
- **AND** o formulário é pré-preenchido com os dados do produto (incluindo categoria, quantidade em estoque e galeria de imagens)
...
O frontend SHALL adicionar em `apps/frontend/src/shared/i18n/messages.pt.ts` e `messages.en.ts` as chaves novas que aparecerem nesta change (ex.: `product.not_found`, rótulos `product.status.active|inactive|draft`, `product.category.invalid`, `product.images.max_items`, `product.images.invalid_url`, mensagens de validação de `quantity`, e mensagens específicas de validação dos novos campos), em português e inglês, reaproveitando chaves já cadastradas em changes anteriores quando aplicável.
...
- **WHEN** validações de `categoryId`, `images` ou `quantity` falham na UI
