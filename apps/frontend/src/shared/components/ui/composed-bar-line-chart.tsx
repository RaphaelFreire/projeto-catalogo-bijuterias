...
type ComposedBarLineChartProps<TData extends ChartDatum> = {
  ...
  onBarClick?: (datum: TData, index: number) => void;
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ top: 8, right: 8, left: 4, bottom: 4 }}>
          ...
          <Bar
            ...
            onClick={
              onBarClick
                ? (datum, index) => onBarClick(datum as unknown as TData, index)
                : undefined
            }
          ...
          >
            {onBarClick
              ? data.map((_, index) => (
                  <Cell
                    key={index}
                    fill={index === selectedIndex ? (selectedBarColor ?? lineColor) : barColor}
                  />
                ))
              : null}
          </Bar>
